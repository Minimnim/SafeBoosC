...
PP = 'C:\Users\PhysioUser\Desktop\Safeboosc\New folder\spo2_postductal_filtered';
...
for k = 6:numel(SS)
    ...
    mydata = table2array(S(k).data);
    FF = fullfile(PP,SS(k).name);
    SS(k).data = readtable(FF);
    filtered = table2array(SS(k).data);
    ts = mydata(:,1);
...
%to extract the intantanous frequency of each epoch in each bandwidth
    ...
    name = append('filtered_features', '_' ,  erase(S(k).name, 'mydata_'));
